...
public class ItemTable {
  ...
  private void loadEtcItems(ResultSet rs) throws SQLException {
    L1EtcItem item = null;
    while (rs.next()) {
      try {
        item = new L1EtcItem();
  ...
  private void loadWeapons(ResultSet rs) throws SQLException {
    ...
    while (rs.next()) {
      try {
        ...
        _allTemplates.put(weapon.getItemId(), weapon);
      } catch (NullPointerException e) {
        _log.error(
            new StringBuilder()
  ...
  private void loadArmors(ResultSet rs) throws SQLException {
    L1Armor armor = null;
    while (rs.next()) {
      try {
        armor = new L1Armor();
        ...
        _allTemplates.put(armor.getItemId(), armor);
      } catch (NullPointerException e) {
        _log.error(
            new StringBuilder()
  ...
  public int findItemIdByNameWithoutSpace(String name, boolean ignoreCase) {
    ...
    int itemid = 0;
    for (L1Item item : _allTemplates.values()) {
      if (item.getName().replace(" ", "").equals(name)
